...
export function printFoldersSidebar(data, i=null, $nodeToPrint=$(".aside")){
    $.each(data, function( index, value ) {
        ...
        if(value.extension=='folder'){
            let $button=`<div id='${index}'><button class="fas fa-chevron-right aside__folder__arrow"></button><button class='aside__folder__name' data-path=${value.path}><i class="far fa-folder"></i> ${value.name}</button></div>`;
